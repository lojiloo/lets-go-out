...
public interface StatsRepository extends JpaRepository<Action, Long> {
    ...
    @Query("select new ru.practicum.dao.QueryResultDto(a.app, a.uri, count(a.ip)) " +
            "from Action as a " +
    ...
    @Query("select new ru.practicum.dao.QueryResultDto(a.app, a.uri, count(DISTINCT a.ip)) " +
            "from Action as a " +
            "where a.timestamp between :start and :end " +
            "group by a.app, a.uri " +
